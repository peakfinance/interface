...
import { METIS_TICKER, PEAK_TICKER, PSHARE_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20;
  if (zappingToken === METIS_TICKER) token = peakFinance.METIS;
  else if (zappingToken === PEAK_TICKER) token = peakFinance.PEAK;
  else if (zappingToken === PSHARE_TICKER) token = peakFinance.PSHARE;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === peakFinance.METIS) return ApprovalState.APPROVED;
  ...
  }, [currentAllowance, pendingApproval, token, peakFinance]);
